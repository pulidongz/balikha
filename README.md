...
`npm run db:seed` creates these accounts. They print at the end of the seed too.

| Role                    | Email                | Password      |
| ----------------------- | -------------------- | ------------- |
| Seller (Maria Ceramics) | `maria@balikha.test` | `password123` |
| Seller (T'boli Weaves)  | `tboli@balikha.test` | `password123` |
| Seller (Narra Studio)   | `narra@balikha.test` | `password123` |
| Buyer                   | `buyer@balikha.test` | `password123` |

⚠️ Dev-only — never use `password123` for anything real. Sign in at <http://localhost:3000/sign-in>.

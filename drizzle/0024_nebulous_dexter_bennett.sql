...
WHERE length("query") > 80
   OR "query" ~* '(\yselect\y.*\yfrom\y|\yunion\y.*\yselect\y|\yinsert\s+into\y|\ydelete\s+from\y|\yupdate\y.*\yset\y|\ydrop\s+(table|database)\y|\yalter\s+table\y|0x[0-9a-f]{4,}|/\*|<script|\y(or|and)\y\s+[0-9]+\s*=\s*[0-9]+)';